...
############################################
#### Running Permutation Data analysis #####
############################################
...
dir = "D:/research/Biometrics_revision"
...
files = list.files(paste(dir, "Real_data_analysis", "Data", "shuffleData", sep = '/'), ".*R")
...
for(i in 1:length(files)){
  ...
  load(paste(dir, "Real_data_analysis", "Data", "shuffleData", files[i], sep = '/'))
  simuparams = str_split(files[i], pattern = "_")[[1]]
  lenreg = as.numeric(simuparams[7])
  min.len = lenreg * 0.20
  simlist0 = list(simlist)
  ...
  results = data.frame(rbind(bump.bench, dmrcat.bench, combp.bench, bfdam1.bench, bfdam2.bench))
  all.results = data.frame(rbind(all.results, results))
}

#### Saving All Results ####

save(all.results, file = paste(dir, "Real_data_analysis", 'Results', 
                               paste("PermutationResults", '.RData', sep = ''), sep = '/'))
